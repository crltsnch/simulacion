...
def f(i, j):
    # Esta función debe ser actualizada según el problema específico
    return 0


for i in range(N):
    w[i][0] = 0  # Frontera inferior
    w[i][M] =  0 #(a+h*i)**2 # Frontera superior/ #recordar que xi= xo(a) + ih
...
for iter in range(100):  # Número de iteraciones
    for i in range(1, N):
        for j in range(1, M):
            w[i][j] = (k**2*(w[i+1][j]+w[i-1][j]) + h**2*(w[i][j+1]+w[i][j-1]) - 2*h**2*k**2*w[i][j]) / (2*(h**2 + k**2))
